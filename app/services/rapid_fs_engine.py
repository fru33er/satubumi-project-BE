...
from app.schemas.rapid_fs import (
    ComponentScores,
    CostBreakdown,
    RapidFSInput,
    RapidFSResult,
...
def calculate_rapid_fs(
    input_data: RapidFSInput, spatial_override: dict[str, Any] = None
) -> RapidFSResult:
    """
    ...
    if spatial_override:
        if "cf" in spatial_override:
            cf = spatial_override["cf"]
        if "er" in spatial_override:
            er = spatial_override["er"]
        if "spatial_overlay_layers" in spatial_override:
            spatial_layers = spatial_override["spatial_overlay_layers"]
...
def generate_recommendations(
    category: str, eco_type: str, area: float, net_revenue: float
) -> list[str]:
    recs = []
