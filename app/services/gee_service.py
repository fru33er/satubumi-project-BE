...
class GEEService:
    ...
    def _init_gee(self):
        try:
            ...
            if (
                ...
                and settings.GEE_PRIVATE_KEY_FILE_PATH
            ):
            ...
            logger.warning(
                f"Gagal menginisialisasi Google Earth Engine: {e}. Dialihkan ke Fallback Mock Engine."
            )
            self.use_mock = True

    def extract_spatial_metrics(
        self, geojson_polygon: dict[str, Any] | None, area_ha: float
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            lc_val = (
                copernicus.reduceRegion(
                    reducer=ee.Reducer.mode(), geometry=ee_geom, scale=100
                )
            ...
            elevation_m = (
                dem.reduceRegion(reducer=ee.Reducer.mean(), geometry=ee_geom, scale=100)
                .get("elevation")
                .getInfo()
                or 120.0
            )
            ...
            is_peatland = (
                True
                if lc_val in [90, 120] or "peat" in landcover_name.lower()
                else False
            )
            ...
            fire_occurrences = (
                modis_fire.count()
                .reduceRegion(reducer=ee.Reducer.mean(), geometry=ee_geom, scale=500)
                .get("BurnDate")
                .getInfo()
                or 0
            )
            ...
            pop_density_img = ee.ImageCollection(
                "CIESIN/GPWv411/GPW_UNWPP_Adjusted_Population_Density"
            ).first()
            pop_density = (
                pop_density_img.reduceRegion(
                    reducer=ee.Reducer.mean(), geometry=ee_geom, scale=1000
                )
                .get("unwpp-adjusted_population_density")
                .getInfo()
                or 12.5
            )

            # 9. Layer Kawasan Hutan (Hansen Global Forest Change Tree Cover 2000)
            hansen = ee.Image("UMD/hansen/global_forest_change_2023_v1_11").select(
                "treecover2000"
            )
            tree_cover_percent = (
                hansen.reduceRegion(
            ...
            forest_status = (
                ...
                if tree_cover_percent > 70
                else "Areal Penggunaan Lain (APL)"
            )

            # Hitung penyesuaian Carbon Factor (CF) dan Emission Reduction (ER) berdasarkan 9 layer overlay
            cf_derived = (
                160.0 if mean_ndvi > 0.6 else (100.0 if mean_ndvi > 0.4 else 60.0)
            )
            ...
            return {
                ...
                "spatial_overlay_layers": {
                    "1_tutupan_lahan": {
                        "value": landcover_name,
                        "fungsi": "Menentukan biomassa dasar",
                    },
                    "2_ndvi": {
                        "value": round(mean_ndvi, 3),
                        "fungsi": "Menentukan kerapatan vegetasi",
                    },
                    "3_ketinggian_m": {
                        "value": round(elevation_m, 1),
                        "fungsi": "Menentukan zona ekologis",
                    },
                    "4_gambut": {
                        "value": is_peatland,
                        "fungsi": "Menentukan potensi karbon tanah",
                    },
                    "5_mangrove": {
                        "value": is_mangrove,
                        "fungsi": "Menentukan potensi karbon biru",
                    },
                    "6_riwayat_kebakaran_count": {
                        "value": fire_occurrences,
                        "fungsi": "Menentukan risiko kebakaran",
                    },
                    "7_aksesibilitas_score": {
                        "value": accessibility_score,
                        "fungsi": "Menentukan kemudahan akses jalan",
                    },
                    "8_kepadatan_penduduk_per_km2": {
                        "value": round(pop_density, 1),
                        "fungsi": "Menentukan risiko sosial",
                    },
                    "9_kawasan_hutan_status": {
                        "value": forest_status,
                        "fungsi": "Menentukan kelayakan legalitas hukum",
                    },
                },
            }
        except Exception as e:
            logger.error(
                f"Error saat merunning GEE query: {e}. Menggunakan fallback mock values."
            )
            return self._get_mock_spatial_metrics(area_ha)
    ...
    def _get_mock_spatial_metrics(self, area_ha: float) -> dict[str, Any]:
        return {
            ...
            "spatial_overlay_layers": {
                "1_tutupan_lahan": {
                    "value": "Hutan Pepohonan Tertutup (Closed Forest)",
                    "fungsi": "Menentukan biomassa dasar",
                },
                ...
                "9_kawasan_hutan_status": {
                    "value": "Kawasan Hutan Produksi Terbatas (HPT)",
                    "fungsi": "Menentukan kelayakan legalitas hukum",
                },
            },
        }
